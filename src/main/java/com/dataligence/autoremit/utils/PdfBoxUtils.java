package com.dataligence.autoremit.utils;
...
public static String generateReceiptFromTransaction(String originalPdf, String targetPdf,Transaction trxn) throws IOException {
	...
	setField("assessRef", trxn.getTransactionRef());
	if(trxn.getPayer() instanceof Individual){
		Individual i = (Individual)trxn.getPayer();
		setField("payer", i.getFirstname()+" "+i.getLastname());
	}else if(trxn.getPayer() instanceof Corporate){
		Corporate c = (Corporate)trxn.getPayer();
		setField("payer", c.getName());
	}
